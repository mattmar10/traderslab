...
type RelativeVolatilityMetricFilterType =
  FilterCriteria["relativeVolatilityMetricFilter"];
...
const RelativeVolatiltyMetricFilter: React.FC<
  RelativeVolatilityMetricFilterProps
> = ({ filterValue, handleFilterChange }) => {
  ...
  const validate = () => {
    const shortEmaValid =
      shortLookback !== undefined &&
      Number.isInteger(shortLookback) &&
      shortLookback > 0;
  ...
  const handleChange = () => {
    if (validate()) {
      handleFilterChange("relativeVolatilityMetricFilter", {
        shortEma: shortLookback,
        longEma: longLookback,
        range,
      });
    }
  };
