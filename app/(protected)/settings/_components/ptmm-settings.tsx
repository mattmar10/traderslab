...
export default function PTMMSettings() {
    ...
    const [settings, setPTMMSettings] =
        useState<PTMMDashboardSettings>(loadPTMMSettingsFromStorage());

    const updateSetting = (
        key: keyof PTMMDashboardSettings,
        value: boolean
    ) => {
    ...
    return (
        <Card className="h-full min-h-[300px] max-h-[30vh] flex flex-col">
            <CardHeader className="flex-none pb-0">
                <CardTitle className="text-xl">Ptmm Dashboard Settings</CardTitle>
                <CardDescription>Configure PTMM dashboard specific settings</CardDescription>
            </CardHeader>
            <CardContent>
                <Accordion type="single" defaultValue="ptmm-settings" className="w-full space-y-4 mt-4">
                    <AccordionItem
                        value="ptmm-settings"
