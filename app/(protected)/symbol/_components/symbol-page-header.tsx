...
const SymbolPageHeader: React.FC<SymbolPageHeaderProps> = ({ quote, profile }) => {
    ...
    const priceString = data && !error ? data.price.toFixed(2) : `${quote.price.toFixed(2)} (Delayed)`
    const currentChange = data && !error ? data.change : quote.change
    const currentChangePercent = data && !error ? data.changesPercentage : quote.changesPercentage
            ...
            <CardContent className="pt-6 pb-4 pl-0 pr-0 flex justify-between">
                <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center justify-between">
                            <div>
                                <h1 className="text-2xl font-bold">{quote.name || quote.symbol}</h1>
                    ...
                    {profile.sector && <Badge className="p-2" variant={"secondary"}>{profile.sector}</Badge>}
                    {profile.industry && <Badge className="p-2" variant={"secondary"}>{profile.industry}</Badge>}
                ...
                <div className="mt-4">
                    <div className="flex items-baseline gap-3">
                        ...
                        <div className={`flex items-center gap-1 text-lg ${isPositiveChange ? 'text-traderslabblue' : 'text-red-500'}`}>
                            {isPositiveChange ? (
