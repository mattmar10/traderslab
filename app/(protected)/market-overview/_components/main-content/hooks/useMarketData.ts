...
import { getIntradayChart, getQuotesFromFMP } from "@/actions/market-data/actions";
...
const useMarketData = (dateToUse: Date) => {
    ...
    const isLoading = quoteLoading || ndx100Data.isLoading || nyseData.isLoading || sp500Data.isLoading || russell2000Data.isLoading;
    const hasError = quoteError || ndx100Data.isError || nyseData.isError || sp500Data.isError || russell2000Data.isError;
    ...
    if (quoteData && !isLoading && !hasError) {
        const marketData = [
            { name: "NDX100", data: ndx100Data.data, openPrice: quoteData.find(q => q.symbol === "QQQE")?.previousClose },
            { name: "NYSE", data: nyseData.data, openPrice: quoteData.find(q => q.symbol === "^NYA")?.previousClose },
            { name: "S&P 500", data: sp500Data.data, openPrice: quoteData.find(q => q.symbol === "RSP")?.previousClose },
            { name: "Russell 2000", data: russell2000Data.data, openPrice: quoteData.find(q => q.symbol === "IWM")?.previousClose },
        ];
