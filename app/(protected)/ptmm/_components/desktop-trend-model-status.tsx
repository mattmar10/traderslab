...
import { PTTrendModelSchema, PTTrendSecondaryState, PTTrendStateStatus } from "@/lib/types/fmp-types";
...
const DesktopTrendModelStatus: FC<DesktopTrendModelStatusProps> = ({ dataset }) => {
            ...
            <div className="flex gap-x-1">
                <div className={makePrimaryOutterStyle(model)}>
                    <div className="flex items-center text-xs font-semibold p-1">
                        {model.toLowerCase().charAt(0).toUpperCase() + model.toLowerCase().slice(1)}
...
const makePrimaryOutterStyle = (status: PTTrendStateStatus) => {
    const baseStyle = "flex items-center justify-center p-1 bg-opacity-10 border rounded shadow-sm"; // Unified base style
...
const makeSecondary = (secondaryOpp: PTTrendSecondaryState) => {
    ...
    const classNameMap: Record<PTTrendSecondaryState, string> = {
        "ST BUYING OPPORTUNITY": "flex items-center justify-center p-1 bg-uptrend/10 border border-uptrend/20 text-uptrend rounded",
        "LT BUYING OPPORTUNITY": "flex items-center justify-center p-1 bg-uptrend/10 border border-uptrend/20 text-uptrend rounded",
        "SHORTING OPPORTUNITY": "flex items-center justify-center p-1 bg-destructive/10 border border-destructive/20 text-destructive rounded",
        "TRIMMING OPPORTUNITY": "flex items-center justify-center p-1 bg-warning/10 border border-warning/20 text-warning rounded",
        "NONE": "flex items-center justify-center p-1 bg-foreground/5 border border-foreground/10 text-foreground/60 rounded"
    };

    return <div className={`text-xs font-semibold ${classNameMap[secondaryOpp]}`}>{labelMap[secondaryOpp]}</div>;
};
