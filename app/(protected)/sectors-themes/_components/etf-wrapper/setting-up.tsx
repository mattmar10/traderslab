...
const SettingUp: React.FC<SettingUpProps> = ({ ticker }) => {
    ...
    const {
        ...
        queryFn: async () => {
            try {
                const result = await getSettingUpStocksForEtf(ticker);
                return result;
            } catch (error) {
    ...
    if (!data || error) {
        return <ErrorCard errorMessage={`Unable to load setting up stocks for ${ticker}`} />
    }
