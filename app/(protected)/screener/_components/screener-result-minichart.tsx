...
import { FMPEarningsCalendar, FMPEarningsCalendarSchema, FMPHistoricalResultsSchema, isFMPDataLoadingError } from "@/lib/types/fmp-types";
...
const ScreenerMiniChartWrapper: React.FC<ScreenerMiniChartWrapperProps> = React.memo(({
    ...
    if (quoteData && quoteData.open && !isFMPDataLoadingError(quoteData) && filteredCandles.length > 0) {  // Add length check
        try {
